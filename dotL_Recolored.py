...
# Маппинг внутренних идентификаторов загрузчиков на Modrinth
LOADER_TO_MODRINTH = {
    "fabric": "fabric",
    "forge": "forge",
    "neoforge": "neoforge",
...
def extract_minecraft_versions(mod_data):
    """Извлекает список версий Minecraft из fabric.mod.json."""
...
def parse_version_range(range_str):
    """Парсит Maven-диапазон версий в список версий."""
...
def parse_toml_mod_info(content, loader):
    """Парсит mods.toml / neoforge.mods.toml и возвращает информацию о моде."""
    if tomllib is None:
        return None
    try:
        ...
        for dep in dep_list:
            ...
            if dep.get("modId") == "minecraft":
                ...
                if mc_vers:
                    loader_ver = None
                    if loader == "forge":
                        loader_ver = data.get("loaderVersion")
                    elif loader == "neoforge":
                        loader_ver = data.get("neoVersion")
                    return {
                        "loader": loader,
                        "mc_versions": mc_vers,
...
def read_mod_info(jar_path):
    """
    Возвращает dict: {'loader': str, 'mc_versions': list, 'loader_version': str|None}
    или None, если это не поддерживаемый мод.
    """
    try:
...
def modrinth_search(query, loader, mc_version, limit=20):
    """Поиск модов на Modrinth. loader — уже modrinth-совместимый."""
    facets = [
        ["project_type:mod"],
        [f"categories:{loader}"],
...
def modrinth_get_versions(project_id, loader, mc_version):
    """Возвращает список версий проекта, совместимых с загрузчиком и MC."""
    params = {
...
class ModrinthDownloadThread(QThread):
    ...
    def __init__(self, projects, mods_dir, loader, mc_version, download_deps):
        super().__init__()
        self.projects = projects  # list of {"project_id":..., "title":...}
...
class ModrinthWindow(QDialog):
    def __init__(self, instance_name, loader_id, mc_version, mods_dir, parent=None):
        ...
        self.chosen = {}        # project_id -> hit dict
        self._result_cards = []
    ...
    def _build_search_page(self):
        ...
        self.search_status.setStyleSheet("color: #888; font-size: 11px;")
        ...
        self.chosen_label.setStyleSheet("color: #ccc;")
        bottom.addWidget(self.chosen_label)
        bottom.addStretch()

        confirm_btn = QPushButton("Подтвердить")
        confirm_btn.setFixedSize(180, 40)
    ...
    def _build_confirm_page(self):
        ...
        self.confirm_label.setStyleSheet("color: #ccc;")
        v.addWidget(self.confirm_label)

        self.confirm_list = QListWidget()
        self.confirm_list.setStyleSheet(
        ...
        self.log_area.setStyleSheet(
            "QTextEdit { background-color: #0a0a0a; color: #00ff00; "
            "border: 1px solid #333; }"
        )
        ...
        self.deps_check.setStyleSheet("color: #ccc; padding: 6px 0;")
    ...
    def _add_result_card(self, hit):
        ...
        author.setStyleSheet("color: #888; font-size: 10px;")
        ...
        meta.setStyleSheet("color: #777; font-size: 10px;")
        info.addWidget(meta)
...
class DotLauncher(QMainWindow):
    ...
    def init_ui(self):
        ...
        memory_label.setStyleSheet("color: #ffffff;")
        ...
        self.managed_java_check.setStyleSheet("color: #ffffff;")
        ...
        instances_label.setStyleSheet("color: #ffffff;")
        left_layout.addWidget(instances_label)

        self.instance_list = QListWidget()
        self.instance_list.setStyleSheet("QListWidget { color: #ffffff; }")
        ...
        self.instance_name_label.setStyleSheet("color: #ffffff;")
        ...
        self.instance_version_label.setStyleSheet("color: #ffffff;")

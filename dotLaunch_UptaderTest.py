...
# ВАЖНО: замени на свои данные перед релизом на GitHub.
GITHUB_OWNER = "your-name"
GITHUB_REPO = "dotLauncher"
UPDATE_ASSET_NAME = "dotLaunch.exe"
UPDATE_API_URL = (
    f"https://api.github.com/repos/{GITHUB_OWNER}/{GITHUB_REPO}/releases/latest"
)
UPDATE_USER_AGENT = f"{APP_NAME}/{LAUNCHER_VERSION} (dotLauncher updater)"
UPDATE_CHECK_INITIAL_DELAY_MS = 5000
UPDATE_CHECK_INTERVAL_MS = 10 * 60 * 1000     # 10 минут между проверками
UPDATE_MIN_INTERVAL_S = 180                   # 3 минуты кэш
...
def version_tuple(v):
    """
    "0.5" → (0, 5, 0); "v0.5.1" → (0, 5, 1).
    Всегда три числа — чтобы tuple'ы разной длины не сравнивались случайно.
    """
...
# ============================================================
#  ОКНО: УВЕДОМЛЕНИЕ ОБ ОБНОВЛЕНИИ
# ============================================================
class UpdateNotifyDialog(QDialog):
    """
    Немодальное по фокусу окно: показывается поверх, но не отбирает
    клавиатуру у игры. Звук — системный QApplication.beep().
    """
    def __init__(self, new_version, body, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Обновление dotLauncher")
        ...
        current = QLabel(f"У вас установлена версия: {LAUNCHER_VERSION}")
        ...
        self.body_edit.setPlainText(body_text if body_text else "(без описания)")
        ...
        cancel_btn = QPushButton("Отмена")
        cancel_btn.setFixedSize(110, 28)
        cancel_btn.clicked.connect(self.reject)
        btns.addWidget(cancel_btn)

        ok_btn = QPushButton("Обновить")
        ok_btn.setFixedSize(110, 28)
...
class UpdateCheckThread(QThread):
    ...
    def run(self):
        try:
            ...
            r = requests.get(UPDATE_API_URL, headers=headers, timeout=15)
            if r.status_code in (403, 429):
                self.finished_signal.emit(False, {}, "rate_limit")
                return
            r.raise_for_status()
            data = r.json()
            if not isinstance(data, dict):
                self.finished_signal.emit(False, {}, "Некорректный ответ")
                return
            self.finished_signal.emit(True, data, "")
        except Exception as e:
            self.finished_signal.emit(False, {}, str(e))
...
# ============================================================
#  ПОТОК: СКАЧИВАНИЕ ОБНОВЛЕНИЯ
# ============================================================
class UpdateDownloadThread(QThread):
    progress_signal = pyqtSignal(int, int)
    finished_signal = pyqtSignal(bool, str, str)  # ok, path, error

    def __init__(self, url, dest_path, expected_size=None, expected_sha256=None):
        super().__init__()
        self.url = url
    ...
    def run(self):
        ...
        try:
            ...
            r = requests.get(self.url, headers=headers, stream=True, timeout=120)
            r.raise_for_status()

            total = int(r.headers.get("Content-Length", 0) or 0)
            if not total and self.expected_size:
                total = int(self.expected_size)
            ...
            if self.expected_size is not None:
                ...
                if actual_size != int(self.expected_size):
                    try:
                        os.remove(tmp)
                    except OSError:
                        pass
                    self.finished_signal.emit(
                        ...
                        f"ожидалось {self.expected_size}, получено {actual_size}"
                    )
                    return
            ...
            if self.expected_sha256:
                ...
                if actual_sha != self.expected_sha256.lower():
                    try:
                        os.remove(tmp)
                    except OSError:
                        pass
                    self.finished_signal.emit(
                        False, "",
                        f"SHA-256 не совпадает: ожидалось {self.expected_sha256}, "
                        f"получено {actual_sha}"
                    )
                    return

            # Проверка MZ-заголовка (это .exe?)
            ...
            if magic != b"MZ":
                ...
                self.finished_signal.emit(
                    False, "",
                    "Скачанный файл не является .exe (нет MZ-заголовка)"
                )
                return
...
class DotLauncher(QMainWindow):
    ...
    def _validate_config(self, cfg):
        ...
        # Ключи апдейтера
        out["check_updates"] = bool(cfg.get("check_updates", True))
        sv = cfg.get("skipped_version")
        if isinstance(sv, str) and sv:
            out["skipped_version"] = sv
        out["last_check_ts"] = safe_int(cfg.get("last_check_ts"), 0)
    ...
    def load_config(self):
        ...
        self.config.setdefault("check_updates", True)
        self.config.setdefault("last_check_ts", 0)
        self.save_config()
        self.init_workspace()
    ...
    def init_workspace(self):
        ...
        # Запускаем апдейтер через таймер
        self.update_timer = QTimer(self)
        self.update_timer.setInterval(UPDATE_CHECK_INTERVAL_MS)
        self.update_timer.timeout.connect(self.check_for_updates)
        self.update_timer.start()
        QTimer.singleShot(UPDATE_CHECK_INITIAL_DELAY_MS, self.check_for_updates)
    ...
    def on_instance_context_menu(self, pos):
        ...
        # --- Пустая область списка: только создание/импорт/проверка обновл. ---
        if item is None:
            act_new = menu.addAction("Новая сборка")
            act_import = menu.addAction("Импорт модпака")
            menu.addSeparator()
            act_check_upd = menu.addAction("Проверить обновления")
    ...
    def check_for_updates(self, manual=False):
        """Запрос к GitHub о новой версии. Можно вызвать вручную."""
        ...
        now = int(time.time())
        if not manual:
            last = safe_int(self.config.get("last_check_ts"), 0)
            if now - last < UPDATE_MIN_INTERVAL_S:
                return
        self.config["last_check_ts"] = now
        self.save_config()

        self.update_check_thread = UpdateCheckThread()
        self.update_check_thread.finished_signal.connect(self.on_update_check_done)
        self.update_check_thread.start()

    def on_update_check_done(self, success, data, error):
        if not success:
            if error == "rate_limit":
                self.log("[Обновление] GitHub rate limit, попробую позже.")
            elif error:
                self.log(f"[Обновление] Не удалось проверить: {error}")
            return
        ...
        if version_tuple(tag) <= version_tuple(LAUNCHER_VERSION):
            return

        skipped = self.config.get("skipped_version", "")
        if skipped and version_tuple(skipped) >= version_tuple(tag):
            return
        ...
        if not asset:
            self.log(
                f"[Обновление] В релизе {tag} нет файла {UPDATE_ASSET_NAME}, "
                f"пропускаю."
            )
            return
        ...
        if self.launcher_thread and self.launcher_thread.isRunning():
            ...
            self.log(
                f"[Обновление] Доступна версия {tag}. Покажу уведомление "
                f"после завершения игры."
            )
            return
    ...
    def show_update_dialog(self, info):
        QApplication.beep()
        dlg = UpdateNotifyDialog(info["tag"], info["body"], parent=self)
        if dlg.exec() == QDialog.DialogCode.Accepted:
            self.start_update_download(info)
        else:
            self.config["skipped_version"] = info["tag"]
            self.save_config()
            self.log(
                f"[Обновление] Пользователь отказался от версии {info['tag']}."
            )
    ...
    def start_update_download(self, info):
        ...
        if not url:
            QMessageBox.warning(
                self, "Ошибка",
                "Не удалось получить ссылку на файл обновления."
            )
            return

        update_dir = os.path.join(
            self.config["workspace"], ".dotLauncher", "update"
        )
        os.makedirs(update_dir, exist_ok=True)
        dest = os.path.join(update_dir, UPDATE_ASSET_NAME)

        expected_sha = None
        digest = info.get("digest")
        if isinstance(digest, str) and digest.startswith("sha256:"):
            expected_sha = digest[len("sha256:"):]

        self.progress_bar.setVisible(True)
        self.progress_bar.setValue(0)
        self.log(
            f"[Обновление] Скачиваю {UPDATE_ASSET_NAME} "
            f"({info.get('size', 0)} байт)..."
        )
        ...
        self.update_download_thread.progress_signal.connect(self.update_progress)
        self.update_download_thread.finished_signal.connect(
            lambda ok, path, err, i=info: self.on_update_download_finished(ok, path, err, i)
        )
        self.update_download_thread.start()

    def on_update_download_finished(self, success, path, error, info):
        self.progress_bar.setVisible(False)
        if not success:
            self.log(f"[Обновление] Ошибка скачивания: {error}")
            QMessageBox.warning(
        ...
        self.log(f"[Обновление] Файл скачан: {path}")

        reply = QMessageBox.question(
            self, "Обновление загружено",
            "Обновление загружено. Лаунчер закроется и перезапустится.\n"
            "Продолжить?",
            QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
        )
        if reply != QMessageBox.StandardButton.Yes:
            self.log("[Обновление] Пользователь отменил установку.")
            return

        self.apply_update_and_restart(path)

    def apply_update_and_restart(self, new_file_path):
        if not sys.platform.startswith("win"):
            QMessageBox.information(
                self, "Недоступно",
                "Автообновление доступно только для Windows-версии."
            )
            return

        # Определяем текущий исполняемый файл (.exe или .py)
        ...
        bat_path = os.path.join(
            tempfile.gettempdir(),
            f"dotlaunch_update_{uuid.uuid4().hex[:8]}.bat",
        )

        try:
            with open(bat_path, "w", encoding="utf-8") as f:
                f.write(UPDATE_BAT_TEMPLATE)
        except Exception as e:
            QMessageBox.warning(
                self, "Ошибка",
        ...
        try:
            ...
            subprocess.Popen(
                ["cmd.exe", "/C", bat_path,
                 str(os.getpid()), target, new_file_path, backup],
        ...
        self.log("[Обновление] Перезапуск через внешний апдейтер...")
        QApplication.quit()
    ...
    def on_launch_finished(self, success, message):
        ...
        # Отложенное уведомление об обновлении (если показывать было нельзя)
        if self.pending_update_info is not None:
            info = self.pending_update_info
            self.pending_update_info = None
            QTimer.singleShot(500, lambda i=info: self.show_update_dialog(i))
    ...
    def closeEvent(self, event):
        ...
        if self.update_download_thread and self.update_download_thread.isRunning():
            reply = QMessageBox.question(
                self, "Скачивание обновления",
                "Идёт скачивание обновления. Прервать и выйти?",
                QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No,
            )
            if reply != QMessageBox.StandardButton.Yes:
                event.ignore()
                return
            self.update_download_thread.stop()
            self.update_download_thread.wait(3000)

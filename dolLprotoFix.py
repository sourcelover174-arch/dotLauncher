...
# ============================================================
def find_java():
    """Пытается найти java в PATH или стандартных местах установки."""
    java = shutil.which("java")
    if java:
        return java

    # Windows: типичные места
    ...
    return "java"  # fallback — путь по умолчанию
...
class LauncherThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.log_signal.emit(
                f"[dotLauncher] Рабочая папка: {minecraft_dir}"
            )
...
class DotLauncher(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.drop_zone.filesDropped.connect(self.handle_dropped_files)  # ВАЖНО
    ...
    def handle_dropped_files(self, files):
        ...
        # ВАЖНО: моды кладём внутрь .minecraft/mods, где их найдёт игра
    ...
    def play_game(self):
        ...
        java_path = find_java()
        self.log(f"[dotLauncher] Java: {java_path}")

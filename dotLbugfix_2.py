...
def find_java(min_major=17):
    """Возвращает (path, major) или (fallback_path, None)."""
    fallback = None
...
class LauncherThread(QThread):
    ...
    def run(self):
        try:
            ...
            self.log_signal.emit(
                f"[dotLauncher] JVM: {self.java_path} ({self.memory_mb} MB)"
            )
...
class DotLauncher(QMainWindow):
    ...
    def _resolve_java(self):
        if self.config.get("java_path") and os.path.isfile(self.config["java_path"]):
            return self.config["java_path"], get_java_major(self.config["java_path"])
        return find_java()
        ...
        if self.launcher_thread and self.launcher_thread.isRunning():
            if self.launcher_thread.process is None:
                ...
                reply = QMessageBox.question(
                    ...
                    "Идёт установка Minecraft/Fabric.\n"

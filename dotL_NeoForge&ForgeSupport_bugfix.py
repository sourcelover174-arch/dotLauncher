...
class LauncherThread(QThread):
    ...
    def run(self):
        try:
            ...
            # 4) Получаем ID установленной версии для запуска
...
class ModpackImportThread(QThread):
    log_signal = pyqtSignal(str)
    ask_signal = pyqtSignal(list, list)  # версии MC, список загрузчиков
    ...
    def run(self):
        try:
            ...
            # Собираем статистику по загрузчикам и версиям
            loaders = Counter()
            versions_per_loader = {}
            loader_versions = {}
            ...
            # Определяем загрузчик: если все моды одного типа — берём его
            if len(loaders) == 1:
                ...
                # Несколько загрузчиков — спрашиваем пользователя
                self.log_signal.emit(
                ...
                chosen_loader_version = loader_versions.get(chosen_loader)
                # Если версия не выбрана, используем первую
                if not chosen_version:
                    chosen_version = mc_versions[0] if mc_versions else None

            # Если загрузчик один, но версий несколько — спрашиваем версию
            if len(loaders) == 1:
                ...
                # Спрашиваем имя, если ещё не задано
...
class DotLauncher(QMainWindow):
    ...
    def on_import_ask(self, versions, loaders):
        """UI-поток: спрашиваем версию, загрузчик и имя."""
        thread = self.import_thread
        if thread is None:
            return

        # Выбор загрузчика, если их несколько
        if len(loaders) > 1:
            ...
            if not ok or not chosen_loader_name:
                thread.cancel()
                return
            # Находим ID загрузчика по имени
            chosen_loader = None
